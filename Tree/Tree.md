...
```
## BFS: 
# Iterate (Binary and kth tree)
# Inorder Traverse
```
Inorder needs to go the left most node first, then turn to the right.
```
```java
class Solution {
...
# PreOrder Traverse
```
root -> left -> right (Since using stack, we should put right in the stack first so we can get it out first.) 
```
```java
class Solution {
...
# PostOrder Traverse
```
Insert the answer in reverse order, and put the children left and right.
root -> right -> left reverse to  left, right, root
```
```java
class Solution {
