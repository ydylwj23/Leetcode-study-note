...
class Solution {
    public int openLock(String[] deadends, String target) {
        //use array to mark visited status
        boolean[] visited = new boolean[10000];
        //mark deadends as visited
        for(var s : deadends){
            visited[Integer.parseInt(s)] = true;
        }
        //if the lock is locked at the begining
        ...
        while(!q.isEmpty()){
            ...
            for(int i = 0; i < size; ++i){
                int cur = q.poll();
                //generate the next number(plus)
                for(int j = 0; j < 4; j++){
                    for(int sign = 0; sign < 2; sign++){
                        int newNum = 0;
                        int digit = (cur % (int)Math.pow(10, j + 1)) / (int)Math.pow(10, j);
                        //plus
                        if(sign == 0){
                            if(digit == 9){
                                newNum = cur - digit * (int)Math.pow(10, j);
                            }
                            else{
                                newNum = cur + (int)Math.pow(10, j);
                            }
                        }
                        //minus
                        else{
                            if(digit == 0){
                                newNum = cur + 9 * (int)Math.pow(10, j);
                            }
                            else{
                                newNum = cur - (int)Math.pow(10, j);
                            }
                        }
                        //if this is the right code
                        if(Integer.parseInt(target) == newNum){
                            return step;
                        }
                        //only add for the next search if it is not visited and is not a dead end
                        if(!visited[newNum]){
                            q.add(newNum);
                            visited[newNum] = true;
                        }
